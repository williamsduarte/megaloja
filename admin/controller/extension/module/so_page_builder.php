...
class ControllerExtensionModuleSopagebuilder extends Controller {
	...
	public function index() {
		...
		/* Load Template */ 
		$this->response->setOutput($this->load->view('extension/module/so_page_builder', $data));
	}
	...
	public function _dataDefaultModule($module_id){	
		...
		$data['moduletabs'] 	= $this->model_setting_module->getModulesByCode( 'so_page_builder' );
		$data['link'] 			= $this->url->link('extension/module/so_page_builder', 'user_token=' . $this->session->data['user_token'] . '', 'SSL');
	...
	public function _getDataModule($module_id){
		...
		if ($module_id != "") {
			...
			$data['action'] = $this->url->link('extension/module/so_page_builder', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $module_id, 'SSL');			
			...
			$data['selectedid'] = 0;
			$data['action'] = $this->url->link('extension/module/so_page_builder', 'user_token=' . $this->session->data['user_token'], 'SSL');
	...
	public function _saveAllModule($module_id,$action){
		...
		if ($module_id == "") {
			...
			$this->model_setting_module->addModule('so_page_builder', $this->request->post);
		} else {
			/* Write File Css */
		...
		switch($action){
			...
			break;
			case "save_edit":
				$this->response->redirect($this->url->link('extension/module/so_page_builder', 'module_id='.$module_id.'&user_token=' . $this->session->data['user_token'], 'SSL'));
			break;
			case "save_new":
				$this->response->redirect($this->url->link('extension/module/so_page_builder', 'user_token=' . $this->session->data['user_token'], 'SSL'));
			break;
		}
		return $module_id;
	}
	...
	public function _duplicateModule($module_id){
		if($this->_checkPermission()){
			...
			$this->response->redirect($this->url->link('extension/module/so_page_builder', 'module_id='.$module_id_dup.'&user_token=' . $this->session->data['user_token'], 'SSL'));				
		}else{
			$this->session->data['warning'] = $this->language->get('error_permission');
		}	
		return $module_id;
	...
	public function _loadStyleAndScript(){
		...
		if ($data['direction'] != 'rtl') $this->document->addStyle('view/javascript/so_page_builder/css/style.css');
		else $this->document->addStyle('view/javascript/so_page_builder/css/style-rtl.css');
		...
		$this->document->addStyle('view/javascript/so_page_builder/css/select2.min.css');
		$this->document->addScript('view/javascript/so_page_builder/js/select2.min.js');
	}
	...
	public function _breadcrumbs($module_id){
		...
		if ($module_id == "") {
			$this->data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/module/so_page_builder', 'user_token=' . $this->session->data['user_token'], 'SSL')
			);
		} else {
			$this->data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/module/so_page_builder', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $module_id, 'SSL')
			);
		}
