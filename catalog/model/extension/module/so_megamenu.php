...
class ModelExtensionModuleSomegamenu extends Model {
    ...
    public function getMenu($module_id) {
		...
		$query = $this->db->query("SELECT c.category_id, c.parent_id, c.image, cd.name, c.category_id as id  FROM " . DB_PREFIX . "category c
		...
		foreach ($query->rows as $row) {
            ...
            if($row['icon']) {
                ...
                $icon = '<img class="lazyload" src="data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw==" data-src="'.$link.'" alt="">';
            }
            $description = false;
	...
	function getCat($cid)
    {
       // $query = $this->db->query("SELECT c.category_id, c.parent_id, c.image, cd.name FROM ".DB_PREFIX."category c INNER JOIN ".DB_PREFIX."category_description cd ON c.category_id = cd.category_id WHERE c.category_id='".$cid."' AND c.status='1' AND cd.language_id = '" . (int)$this->config->get('config_language_id') . "'  ");
        // return $query->row;
